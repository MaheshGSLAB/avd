...
class AaaRootBlock(CliSection):
    ...
    def _generate(self) -> None:
        aaa_root = self.data.aaa_root
        if aaa_root.disabled:
            self._header("no aaa root")
            return
        sha512_password = aaa_root.secret.sha512_password
        if not sha512_password:
            return
        password = hide_passwords(sha512_password, self.data.eos_cli_config_gen_configuration.hide_passwords)
        self._header(f"aaa root secret sha512 {password}")
