...
class EthernetInterfacesGenerator(CliGenerator):
    ...
    def ethernet_interfaces(self) -> None:
        ...
        for intf in natural_sort(self.inputs.ethernet_interfaces or [], sort_key="name"):
            self._model.extend(EthernetInterface(intf, self.inputs).render())
...
class EthernetInterface(CliSection):
    ...
    def _section(self) -> None:
        intf = self.intf
        self._section_heading(f"interface {intf.name}")

        # Comment lines (J2 11-15)
        if intf.comment:
            for line in intf.comment.splitlines():
                self._cli_line(f"!! {line}")

        self._cli_line("profile {}", intf.profile)
        self._cli_line("description {}", intf.description)

        # shutdown / no shutdown (J2 22-26)
        if intf.shutdown is True:
            self._cli_line("shutdown")
        elif intf.shutdown is False:
            self._cli_line("no shutdown")

        self._cli_line("load-interval {}", intf.load_interval)
        self._cli_line("mtu {}", intf.mtu)

        # logging event link-status (J2 33-37)
        if intf.logging.event.link_status is True:
            self._cli_line("logging event link-status")
        elif intf.logging.event.link_status is False:
            self._cli_line("no logging event link-status")

        self._cli_line("traffic-policy input {}", intf.traffic_policy.input)
        self._cli_line("traffic-policy output {}", intf.traffic_policy.output)
        self._cli_line("bgp session tracker {}", intf.bgp.session_tracker)
        self._cli_line("l2-protocol forwarding profile {}", intf.l2_protocol.forwarding_profile)
        self._cli_line("flowcontrol receive {}", intf.flowcontrol.received)
        self._cli_line("l2 mtu {}", intf.l2_mtu)
        self._cli_line("l2 mru {}", intf.l2_mru)

        # logging event congestion-drops (J2 59-63)
        if intf.logging.event.congestion_drops is True:
            self._cli_line("logging event congestion-drops")
        elif intf.logging.event.congestion_drops is False:
            self._cli_line("no logging event congestion-drops")

        self._cli_line("speed {}", intf.speed)
        self._render_error_correction_encoding(intf)
        self._render_switchport(intf)
        self._render_encapsulation_dot1q(intf)
        self._cli_line("vlan id {}", intf.vlan_id)
        self._sub_section(EthernetInterfaceEncapsulationVlan(intf))
        self._cli_line("switchport source-interface {}", intf.switchport.source_interface)
        self._render_vlan_translations(intf)
        self._cli_line("l2-protocol encapsulation dot1q vlan {}", intf.l2_protocol.encapsulation_dot1q_vlan)
        self._cli_line("mac timestamp {}", intf.mac_timestamp)
        self._sub_section(EthernetInterfaceAddressLocking(intf))
        self._sub_section(EthernetInterfaceEvpnEthernetSegment(intf))
        self._cli_line("flow tracker hardware {}", intf.flow_tracker.hardware)
        self._cli_line("flow tracker sampled {}", intf.flow_tracker.sampled)

        # snmp trap link-change (J2 308-312)
        if intf.snmp_trap_link_change is False:
            self._cli_line("no snmp trap link-change")
        elif intf.snmp_trap_link_change is True:
            self._cli_line("snmp trap link-change")

        self._cli_line("vrf {}", intf.vrf)
        self._render_ip_address(intf)

        if intf.ip_proxy_arp is True:
            self._cli_line("ip proxy-arp")
        if intf.arp_gratuitous_accept is True:
            self._cli_line("arp gratuitous accept")
        if intf.ip_address == "dhcp" and intf.dhcp_client_accept_default_route is True:
            self._cli_line("dhcp client accept default-route")

        self._cli_line("ip verify unicast source reachable-via {}", intf.ip_verify_unicast_source_reachable_via)
        ...
        if intf.dhcp_server_ipv4 is True:
            self._cli_line("dhcp server ipv4")
        if intf.dhcp_server_ipv6 is True:
            self._cli_line("dhcp server ipv6")
        ...
        self._cli_line("ip access-group {} in", intf.access_group_in)
        self._cli_line("ip access-group {} out", intf.access_group_out)
        self._cli_line("ipv6 access-group {} in", intf.ipv6_access_group_in)
        self._cli_line("ipv6 access-group {} out", intf.ipv6_access_group_out)
        self._cli_line("mac access-group {} in", intf.mac_access_group_in)
        self._cli_line("mac access-group {} out", intf.mac_access_group_out)

        self._render_mpls_ldp(intf)

        # lldp (J2 514-522)
        if intf.lldp.transmit is False:
            self._cli_line("no lldp transmit")
        if intf.lldp.receive is False:
            self._cli_line("no lldp receive")
        self._cli_line("lldp tlv transmit ztp vlan {}", intf.lldp.ztp_vlan)

        # loop protection (J2 523-527)
        if intf.loop_protection is False:
            self._cli_line("no loop-protection")
        elif intf.loop_protection is True:
            self._cli_line("loop-protection")

        self._cli_line("mac security profile {}", intf.mac_security.profile)
        self._render_multicast(intf)

        # mpls ip (J2 549-553)
        if intf.mpls.ip is True:
            self._cli_line("mpls ip")
        elif intf.mpls.ip is False:
            self._cli_line("no mpls ip")

        self._render_ip_nat(intf)

        # ntp serve (J2 561-567)
        if intf.ntp_serve is True:
            self._cli_line("ntp serve")
        elif intf.ntp_serve is False:
            self._cli_line("no ntp serve")

        self._render_ospf(intf)
        self._cli_line("service-policy type pbr input {}", intf.service_policy.pbr.input)
        self._render_pim(intf)
        self._render_poe(intf)
        self._render_port_security(intf)
        self._render_ptp(intf)
        self._cli_line("service-policy type qos input {}", intf.service_policy.qos.input)
        self._cli_line("service-profile {}", intf.service_profile)
        self._render_qos(intf)
        self._cli_line("shape rate {}", intf.shape.rate)
        ...
        if intf.monitor_link_flap_profiles:
            ...
            self._cli_line(f"monitor link-flap profiles {profiles}")
    ...
    def _render_error_correction_encoding(self, intf: Any) -> None:
        ...
        if error_correction_encoding.enabled is False:
            self._cli_line("no error-correction encoding")
        else:
            if error_correction_encoding.fire_code is True:
                self._cli_line("error-correction encoding fire-code")
            elif error_correction_encoding.fire_code is False:
                self._cli_line("no error-correction encoding fire-code")
            if error_correction_encoding.reed_solomon is True:
                self._cli_line("error-correction encoding reed-solomon")
            elif error_correction_encoding.reed_solomon is False:
                self._cli_line("no error-correction encoding reed-solomon")
    ...
    def _render_switchport(self, intf: Any) -> None:
        ...
        if switchport.trunk.private_vlan_secondary is True:
            self._cli_line("switchport trunk private-vlan secondary")
        self._cli_line("switchport pvlan mapping {}", switchport.pvlan_mapping)
        self._cli_line("switchport access vlan {}", switchport.access_vlan)
        if switchport.trunk.native_vlan_tag is True:
            self._cli_line("switchport trunk native vlan tag")
        elif switchport.trunk.native_vlan:
            self._cli_line("switchport trunk native vlan {}", switchport.trunk.native_vlan)
        self._cli_line("switchport phone vlan {}", switchport.phone.vlan)
        self._cli_line("switchport phone trunk {}", switchport.phone.trunk)
        if switchport.vlan_translations.in_required is True:
            self._cli_line("switchport vlan translation in required")
        if switchport.vlan_translations.out_required is True:
            self._cli_line("switchport vlan translation out required")
        self._cli_line("switchport dot1q vlan tag {}", switchport.dot1q.vlan_tag)
        self._cli_line("switchport trunk allowed vlan {}", switchport.trunk.allowed_vlan)
        self._cli_line("switchport mode {}", switchport.mode)
        self._cli_line("switchport dot1q ethertype {}", switchport.dot1q.ethertype)
        if switchport.vlan_forwarding_accept_all is True:
            self._cli_line("switchport vlan forwarding accept all")
        for trunk_group in natural_sort(switchport.trunk.groups or [], ignore_case=False):
            self._cli_line(f"switchport trunk group {trunk_group}")
        if switchport.enabled is True:
            self._cli_line("switchport")
        elif switchport.enabled is False:
            self._cli_line("no switchport")
    ...
    def _render_ip_address(self, intf: Any) -> None:
        ...
        if intf.ip_address is None:
            return
        self._cli_line(f"ip address {intf.ip_address}")
        for ip_secondary in natural_sort(intf.ip_address_secondaries or []):
            self._cli_line(f"ip address {ip_secondary} secondary")

    def _render_ipv6_nd_cache(self, intf: Any) -> None:
        """Render IPv6 ND cache configuration (J2 334-344)."""
        cache = intf.ipv6_nd.cache
        self._cli_line("ipv6 nd cache expire {}", cache.expire)
        self._cli_line("ipv6 nd cache dynamic capacity {}", cache.dynamic_capacity)
        if cache.refresh_always is True:
            self._cli_line("ipv6 nd cache refresh always")
    ...
    def _render_bfd_interface(self, intf: Any) -> None:
        ...
        if bfd.interval and bfd.min_rx and bfd.multiplier:
            self._cli_line(f"bfd interval {bfd.interval} min-rx {bfd.min_rx} multiplier {bfd.multiplier}")
        if bfd.echo is True:
            self._cli_line("bfd echo")
        elif bfd.echo is False:
            self._cli_line("no bfd echo")
    ...
    def _render_ip_igmp_host_proxy(self, intf: Any) -> None:
        ...
        self._cli_line(host_proxy_cli)

        for group in proxy.groups or []:
            if group.exclude or group.include:
                for include_source in group.include or []:
                    self._cli_line(f"{host_proxy_cli} {group.group} include {include_source.source}")
                for exclude_source in group.exclude or []:
                    self._cli_line(f"{host_proxy_cli} {group.group} exclude {exclude_source.source}")
            elif group.group:
                self._cli_line(f"{host_proxy_cli} {group.group}")

        for access_list in proxy.access_lists or []:
            self._cli_line(f"{host_proxy_cli} access-list {access_list.name}")

        if proxy.report_interval:
            self._cli_line(f"{host_proxy_cli} report-interval {proxy.report_interval}")
        if proxy.version:
            self._cli_line(f"{host_proxy_cli} version {proxy.version}")

    def _render_ipv6(self, intf: Any) -> None:
        """Render IPv6 address and ND configuration (J2 419-462)."""
        if intf.ipv6_enable is True:
            self._cli_line("ipv6 enable")

        # ipv6 address (J2 422-430)
        if intf.ipv6_addresses:
            for addr in natural_sort(intf.ipv6_addresses):
                self._cli_line(f"ipv6 address {addr}")
        elif intf.ipv6_address_auto_config is True:
            self._cli_line("ipv6 address auto-config")
        elif intf.ipv6_address:
            self._cli_line(f"ipv6 address {intf.ipv6_address}")

        if intf.ipv6_address_link_local:
            self._cli_line(f"ipv6 address {intf.ipv6_address_link_local} link-local")

        ipv6_nd = intf.ipv6_nd
        if ipv6_nd.ra.rx_accept.default_route is True:
            self._cli_line("ipv6 nd ra rx accept default-route")
        if ipv6_nd.ra.rx_accept.route_preference is True:
            self._cli_line("ipv6 nd ra rx accept route-preference")
        if ipv6_nd.ra.disabled is True or intf.ipv6_nd_ra_disabled is True:
            self._cli_line("ipv6 nd ra disabled")
        if ipv6_nd.managed_config_flag is True or intf.ipv6_nd_managed_config_flag is True:
            self._cli_line("ipv6 nd managed-config-flag")
        if ipv6_nd.other_config_flag is True:
            self._cli_line("ipv6 nd other-config-flag")
    ...
    def _render_channel_group(self, intf: Any) -> None:
        ...
        if channel_group.id is None or channel_group.mode is None:
            return
        self._cli_line(f"channel-group {channel_group.id} mode {channel_group.mode}")
        self._cli_line("lacp timer {}", intf.lacp_timer.mode)
        self._cli_line("lacp timer multiplier {}", intf.lacp_timer.multiplier)
        self._cli_line("lacp port-priority {}", intf.lacp_port_priority)

    def _render_mpls_ldp(self, intf: Any) -> None:
        """Render MPLS LDP configuration (J2 506-512)."""
        if intf.mpls.ldp.igp_sync is True:
            self._cli_line("mpls ldp igp sync")
        if intf.mpls.ldp.interface is True:
            self._cli_line("mpls ldp interface")
        elif intf.mpls.ldp.interface is False:
            self._cli_line("no mpls ldp interface")
    ...
    def _render_multicast(self, intf: Any) -> None:
        ...
        for boundary in multicast.ipv6.boundaries or []:
            self._cli_line(f"multicast ipv6 boundary {boundary.boundary} out")
        if multicast.ipv4.static is True:
            self._cli_line("multicast ipv4 static")
        if multicast.ipv6.static is True:
            self._cli_line("multicast ipv6 static")
    ...
    def _render_ip_nat(self, intf: Any) -> None:
        ...
        for entry in natural_sort(unsorted_nat_entries, sort_key="sort_key"):
            self._cli_line(entry["cli"])

        self._cli_line("ip nat service-profile {}", ip_nat.service_profile)

    def _render_ospf(self, intf: Any) -> None:
        """Render OSPF configuration (J2 568-589)."""
        self._cli_line("ip ospf cost {}", intf.ospf_cost)
        if intf.ospf_network_point_to_point is True:
            self._cli_line("ip ospf network point-to-point")
        if intf.ospf_authentication == "simple":
            self._cli_line("ip ospf authentication")
        elif intf.ospf_authentication == "message-digest":
            self._cli_line("ip ospf authentication message-digest")
        if intf.ospf_authentication_key:
            key_type = intf.ospf_authentication_key_type or "7"
            key = hide_passwords(intf.ospf_authentication_key, self.inputs.eos_cli_config_gen_configuration.hide_passwords)
            self._cli_line(f"ip ospf authentication-key {key_type} {key}")
        self._cli_line("ip ospf area {}", intf.ospf_area)
        for key in natural_sort(intf.ospf_message_digest_keys or [], sort_key="id"):
            if key.hash_algorithm and key.key:
                ...
                masked_key = hide_passwords(key.key, self.inputs.eos_cli_config_gen_configuration.hide_passwords)
                self._cli_line(f"ip ospf message-digest-key {key.id} {key.hash_algorithm} {key_type} {masked_key}")

    def _render_pim(self, intf: Any) -> None:
        """Render PIM IPv4 configuration (J2 593-616)."""
        pim = intf.pim.ipv4
        if pim.sparse_mode is True:
            self._cli_line("pim ipv4 sparse-mode")
        if pim.bidirectional is True:
            self._cli_line("pim ipv4 bidirectional")
        if pim.border_router is True:
            self._cli_line("pim ipv4 border-router")
        self._cli_line("pim ipv4 hello interval {}", pim.hello.interval)
        self._cli_line("pim ipv4 hello count {}", pim.hello.count)
        self._cli_line("pim ipv4 dr-priority {}", pim.dr_priority)
        self._cli_line("pim ipv4 neighbor filter {}", pim.neighbor_filter)
        if pim.bfd is True:
            self._cli_line("pim ipv4 bfd")

    def _render_poe(self, intf: Any) -> None:
        """Render Power over Ethernet configuration (J2 617-652)."""
        poe = intf.poe
        self._cli_line("poe priority {}", poe.priority)
        self._cli_line("poe reboot action {}", poe.reboot.action)
        if poe.link_down.action:
            ...
            self._cli_line(cli)
        self._cli_line("poe shutdown action {}", poe.shutdown.action)
        if poe.disabled is True:
            self._cli_line("poe disabled")
        if poe.limit:
            ...
            if poe_limit_cli and poe.limit.fixed is True:
                poe_limit_cli += " fixed"
            if poe_limit_cli:
                self._cli_line(poe_limit_cli)
        if poe.negotiation_lldp is False:
            self._cli_line("poe negotiation lldp disabled")
        if poe.legacy_detect is True:
            self._cli_line("poe legacy detect")
    ...
    def _render_port_security(self, intf: Any) -> None:
        ...
        if port_security.enabled is True or port_security.violation.mode == "shutdown":
            self._cli_line("switchport port-security")
        elif port_security.violation.mode == "protect":
            if port_security.violation.protect_log is True:
                self._cli_line("switchport port-security violation protect log")
            else:
                self._cli_line("switchport port-security violation protect")

        if port_security.mac_address_maximum.disabled is True:
            self._cli_line("switchport port-security mac-address maximum disabled")
        elif port_security.mac_address_maximum.disabled is False:
            self._cli_line("no switchport port-security mac-address maximum disabled")
        elif port_security.mac_address_maximum.limit:
            self._cli_line(f"switchport port-security mac-address maximum {port_security.mac_address_maximum.limit}")
    ...
    def _render_ptp(self, intf: Any) -> None:
        ...
        if ptp.enable is True:
            self._cli_line("ptp enable")
        self._cli_line("ptp announce interval {}", ptp.announce.interval)
        self._cli_line("ptp announce timeout {}", ptp.announce.timeout)
        self._cli_line("ptp delay-mechanism {}", ptp.delay_mechanism)
        self._cli_line("ptp delay-req interval {}", ptp.delay_req)
        self._cli_line("ptp profile g8275.1 destination mac-address {}", ptp.profile.g8275_1.destination_mac_address)
        self._cli_line("ptp role {}", ptp.role)
        self._cli_line("ptp sync-message interval {}", ptp.sync_message.interval)
        self._cli_line("ptp transport {}", ptp.transport)
        self._cli_line("ptp vlan {}", ptp.vlan)

    def _render_qos(self, intf: Any) -> None:
        """Render QoS trust and marking configuration (J2 724-735)."""
        if intf.qos.trust == "disabled":
            self._cli_line("no qos trust")
        elif intf.qos.trust:
            self._cli_line(f"qos trust {intf.qos.trust}")
        self._cli_line("qos cos {}", intf.qos.cos)
        self._cli_line("qos dscp {}", intf.qos.dscp)
    ...
    def _render_priority_flow_control(self, intf: Any) -> None:
        ...
        if intf.priority_flow_control.enabled is True:
            self._cli_line("priority-flow-control on")
        elif intf.priority_flow_control.enabled is False:
            self._cli_line("no priority-flow-control")
        for priority_block in natural_sort(intf.priority_flow_control.priorities or [], sort_key="priority"):
            if priority_block.no_drop is True:
                self._cli_line(f"priority-flow-control priority {priority_block.priority} no-drop")
            elif priority_block.no_drop is False:
                self._cli_line(f"priority-flow-control priority {priority_block.priority} drop")
    ...
    def _render_tx_queues(self, intf: Any) -> None:
        ...
        for tx_queue in natural_sort(intf.tx_queues or [], sort_key="id"):
            self._sub_section(EthernetInterfaceTxQueue(tx_queue))
    ...
    def _render_uc_tx_queues(self, intf: Any) -> None:
        ...
        for uc_tx_queue in natural_sort(intf.uc_tx_queues or [], sort_key="id"):
            self._sub_section(EthernetInterfaceUcTxQueue(uc_tx_queue))
    ...
    def _render_sflow(self, intf: Any) -> None:
        ...
        if sflow.enable is True:
            self._cli_line("sflow enable")
        elif sflow.enable is False:
            self._cli_line("no sflow enable")
        if sflow.egress.enable is True:
            self._cli_line("sflow egress enable")
        elif sflow.egress.enable is False:
            self._cli_line("no sflow egress enable")
        if sflow.egress.unmodified_enable is True:
            self._cli_line("sflow egress unmodified enable")
        elif sflow.egress.unmodified_enable is False:
            self._cli_line("no sflow egress unmodified enable")

    def _render_isis(self, intf: Any) -> None:
        """Render IS-IS configuration (J2 775-884)."""
        self._cli_line("isis enable {}", intf.isis_enable)
        if intf.isis_bfd is True:
            self._cli_line("isis bfd")
        self._cli_line("isis circuit-type {}", intf.isis_circuit_type)
        self._cli_line("isis metric {}", intf.isis_metric)
        if intf.isis_passive is True:
            self._cli_line("isis passive")
        if intf.isis_hello_padding is False:
            self._cli_line("no isis hello padding")
        elif intf.isis_hello_padding is True:
            self._cli_line("isis hello padding")
        if intf.isis_network_point_to_point is True:
            self._cli_line("isis network point-to-point")
        ...
        def _add_key_id(auth_key: object, suffix: str = "") -> None:
            if auth_key.rfc_5310 is True:  # type: ignore[union-attr]
                key_cli = (
                    ...
                    f"{hide_passwords(auth_key.key, self.inputs.eos_cli_config_gen_configuration.hide_passwords)}"  # type: ignore[union-attr]
                )
            else:
                key_cli = (
                    f"isis authentication key-id {auth_key.id} algorithm {auth_key.algorithm}"  # type: ignore[union-attr]
    ...
    def _render_isis_authentication_keys(self, isis_auth: Any) -> None:
        ...
        both = isis_auth.both
        hide_passwords_enabled = self.inputs.eos_cli_config_gen_configuration.hide_passwords
        if both.key_type and both.key:
            self._cli_line(f"isis authentication key {both.key_type} {hide_passwords(both.key, hide_passwords_enabled)}")
        else:
            level_1 = isis_auth.level_1
            if level_1.key_type and level_1.key:
                self._cli_line(f"isis authentication key {level_1.key_type} {hide_passwords(level_1.key, hide_passwords_enabled)} level-1")
            level_2 = isis_auth.level_2
            if level_2.key_type and level_2.key:
                self._cli_line(f"isis authentication key {level_2.key_type} {hide_passwords(level_2.key, hide_passwords_enabled)} level-2")
    ...
    def _render_storm_control(self, intf: Any) -> None:
        ...
        for section_name in natural_sort(["broadcast", "multicast", "unknown_unicast"]):
            ...
            if section.level:
                ...
                if section.unit == "pps":
                    self._cli_line(f"storm-control {section_cli} level pps {section.level}")
                else:
                    self._cli_line(f"storm-control {section_cli} level {section.level}")
        if storm_control.all.level:
            if storm_control.all.unit == "pps":
                self._cli_line(f"storm-control all level pps {storm_control.all.level}")
            else:
                self._cli_line(f"storm-control all level {storm_control.all.level}")
    ...
    def _render_spanning_tree(self, intf: Any) -> None:
        ...
        if intf.spanning_tree_portfast == "edge":
            self._cli_line("spanning-tree portfast")
        elif intf.spanning_tree_portfast == "network":
            self._cli_line("spanning-tree portfast network")

        self._cli_line("spanning-tree link-type {}", intf.spanning_tree_link_type)

        if intf.spanning_tree_bpduguard and intf.spanning_tree_bpduguard in (True, "True", "enabled"):
            self._cli_line("spanning-tree bpduguard enable")
        elif intf.spanning_tree_bpduguard == "disabled":
            self._cli_line("spanning-tree bpduguard disable")

        if intf.spanning_tree_bpdufilter and intf.spanning_tree_bpdufilter in (True, "True", "enabled"):
            self._cli_line("spanning-tree bpdufilter enable")
        elif intf.spanning_tree_bpdufilter == "disabled":
            self._cli_line("spanning-tree bpdufilter disable")

        if intf.spanning_tree_guard == "disabled":
            self._cli_line("spanning-tree guard none")
        elif intf.spanning_tree_guard:
            self._cli_line(f"spanning-tree guard {intf.spanning_tree_guard}")

        if intf.spanning_tree_bpduguard_rate_limit.enabled is True:
            self._cli_line("spanning-tree bpduguard rate-limit enable")
        elif intf.spanning_tree_bpduguard_rate_limit.enabled is False:
            self._cli_line("spanning-tree bpduguard rate-limit disable")
        ...
        if intf.logging.event.spanning_tree is True:
            self._cli_line("logging event spanning-tree")
        elif intf.logging.event.spanning_tree is False:
            self._cli_line("no logging event spanning-tree")
    ...
    def _render_backup_link(self, intf: Any) -> None:
        ...
        self._cli_line(cli)
        self._cli_line("switchport backup preemption-delay {}", switchport.backup.preemption_delay)
        self._cli_line("switchport backup mac-move-burst {}", switchport.backup.mac_move_burst)
        self._cli_line("switchport backup mac-move-burst-interval {}", switchport.backup.mac_move_burst_interval)
        self._cli_line("switchport backup initial-mac-move-delay {}", switchport.backup.initial_mac_move_delay)
        self._cli_line("switchport backup dest-macaddr {}", switchport.backup.dest_macaddr)
    ...
    def _render_tap_tool(self, intf: Any) -> None:
        ...
        # tap settings
        self._cli_line("switchport tap native vlan {}", switchport.tap.native_vlan)
        ...
        if switchport.tap.encapsulation.vxlan_strip is True and switchport.tap.mpls_pop_all is not True:
            self._cli_line("switchport tap encapsulation vxlan strip")
        ...
        if switchport.tap.encapsulation.gre.strip is True:
            self._cli_line("switchport tap encapsulation gre strip")
        ...
        for destination in natural_sort(switchport.tap.encapsulation.gre.destinations or [], sort_key="destination"):
            ...
            for dest_protocol in natural_sort(destination.protocols or [], sort_key="protocol"):
                if dest_protocol.strip is True:
                    ...
                    self._cli_line(proto_cli)
            if destination.strip is True:
                self._cli_line(f"{tap_enc_cli} strip")

        if switchport.tap.mpls_pop_all is True:
            self._cli_line("switchport tap mpls pop all")

        # tool settings
        if switchport.tool.mpls_pop_all is True:
            self._cli_line("switchport tool mpls pop all")
        if switchport.tool.encapsulation.vn_tag_strip is True:
            self._cli_line("switchport tool encapsulation vn-tag strip")
        if switchport.tool.encapsulation.dot1br_strip is True:
            self._cli_line("switchport tool encapsulation dot1br strip")

        self._cli_line("switchport tap allowed vlan {}", switchport.tap.allowed_vlan)
        self._cli_line("switchport tool allowed vlan {}", switchport.tool.allowed_vlan)

        self._cli_line("switchport tool identity {}", switchport.tool.identity.tag)
        if switchport.tool.identity.dot1q_dzgre_source:
            self._cli_line(f"switchport tool identity dot1q source dzgre {switchport.tool.identity.dot1q_dzgre_source}")
        elif switchport.tool.identity.qinq_dzgre_source:
            self._cli_line(f"switchport tool identity qinq source dzgre {switchport.tool.identity.qinq_dzgre_source}")
        ...
        if switchport.tap.default.groups:
            ...
            self._cli_line(f"switchport tap default group {groups}")
        ...
        if switchport.tap.default.nexthop_groups:
            ...
            self._cli_line(f"switchport tap default nexthop-group {nxhop_groups}")

        for interface in natural_sort(switchport.tap.default.interfaces or []):
            self._cli_line(f"switchport tap default interface {interface}")

        if switchport.tool.groups:
            tool_groups = " ".join(natural_sort(switchport.tool.groups, ignore_case=False))
            self._cli_line(f"switchport tool group set {tool_groups}")

        self._cli_line("switchport tool dot1q remove outer {}", switchport.tool.dot1q_remove_outer_vlan_tag)
    ...
    def _render_traffic_engineering(self, intf: Any) -> None:
        ...
        if traffic_engineering.enabled is True:
            self._cli_line("traffic-engineering")
        if traffic_engineering.bandwidth:
            self._cli_line(f"traffic-engineering bandwidth {traffic_engineering.bandwidth.number} {traffic_engineering.bandwidth.unit}")
        if traffic_engineering.administrative_groups:
            self._cli_line(f"traffic-engineering administrative-group {','.join(traffic_engineering.administrative_groups)}")
        for srlg in natural_sort(traffic_engineering.srlgs or [], ignore_case=False):
            self._cli_line(f"traffic-engineering srlg {srlg}")
        self._cli_line("traffic-engineering metric {}", traffic_engineering.metric)
        if traffic_engineering.min_delay_static:
            self._cli_line(f"traffic-engineering min-delay static {traffic_engineering.min_delay_static.number} {traffic_engineering.min_delay_static.unit}")
            ...
            self._cli_line(f"traffic-engineering min-delay dynamic twamp-light fallback {fallback.number} {fallback.unit}")
    ...
    def _render_link_tracking(self, intf: Any) -> None:
        ...
        for group in intf.link_tracking_groups or []:
            self._cli_line(f"link tracking group {group.name} {group.direction}")
        if intf.link_tracking.direction and intf.link_tracking.groups:
            for group_name in intf.link_tracking.groups:
                self._cli_line(f"link tracking group {group_name} {intf.link_tracking.direction}")
    ...
    def _render_vrrp(self, intf: Any) -> None:
        ...
        for vrid in natural_sort(intf.vrrp_ids or [], sort_key="id"):
            vrrp_id = vrid.id
            self._cli_line("vrrp {} priority-level {}", vrrp_id, vrid.priority_level)
            self._cli_line("vrrp {} advertisement interval {}", vrrp_id, vrid.advertisement.interval)
            ...
            if vrid.preempt.enabled is True and (vrid.preempt.delay.minimum or vrid.preempt.delay.reload):
                ...
                self._cli_line(delay_cli)
            elif vrid.preempt.enabled is False:
                self._cli_line(f"no vrrp {vrrp_id} preempt")

            self._cli_line("vrrp {} timers delay reload {}", vrrp_id, vrid.timers.delay.reload)
            ...
            if vrid.peer_authentication:
                ...
                self._cli_line(peer_auth_cli)

            self._cli_line("vrrp {} ipv4 {}", vrrp_id, vrid.ipv4.address)
            for secondary_ip in natural_sort(vrid.ipv4.secondary_addresses or []):
                self._cli_line(f"vrrp {vrrp_id} ipv4 {secondary_ip} secondary")
            self._cli_line("vrrp {} ipv4 version {}", vrrp_id, vrid.ipv4.version)

            for ipv6_address in natural_sort(vrid.ipv6.addresses or []):
                self._cli_line(f"vrrp {vrrp_id} ipv6 {ipv6_address}")
            ...
            for tracked_obj in natural_sort(vrid.tracked_object or [], sort_key="name", ignore_case=False):
                if tracked_obj.name:
                    ...
                    self._cli_line(tracked_cli)

    def _render_transceiver(self, intf: Any) -> None:
        """Render transceiver configuration (J2 1179-1207)."""
        transceiver = intf.transceiver
        self._cli_line("transceiver media override {}", transceiver.media.override)
        if transceiver.power.ignore is True:
            self._cli_line("transceiver power ignore")
        self._cli_line("transceiver application override {}", transceiver.application_override)
        ...
        if transceiver.transmitter.signal_power:
            self._cli_line(f"transceiver transmitter signal-power {float(transceiver.transmitter.signal_power):.2f}")
        if transceiver.transmitter.disabled is True:
            self._cli_line("transceiver transmitter disabled")
    ...
    def _render_dot1x(self, intf: Any) -> None:
        ...
        if dot1x.pae.mode == "authenticator":
            self._cli_line(f"dot1x pae {dot1x.pae.mode}")
        elif dot1x.pae.mode == "supplicant" and dot1x.pae.supplicant_profile:
            self._cli_line(f"dot1x pae {dot1x.pae.mode} {dot1x.pae.supplicant_profile}")
        ...
        if dot1x.authentication_failure:
            if dot1x.authentication_failure.action == "allow" and dot1x.authentication_failure.allow_vlan:
                self._cli_line(f"dot1x authentication failure action traffic allow vlan {dot1x.authentication_failure.allow_vlan}")
            elif dot1x.authentication_failure.action == "drop":
                self._cli_line("dot1x authentication failure action traffic drop")
        ...
        if dot1x.reauthentication is True:
            self._cli_line("dot1x reauthentication")
        self._cli_line("dot1x port-control {}", dot1x.port_control)

        if dot1x.port_control_force_authorized_phone is True:
            self._cli_line("dot1x port-control force-authorized phone")
        elif dot1x.port_control_force_authorized_phone is False:
            self._cli_line("no dot1x port-control force-authorized phone")

        # host-mode (J2 1286-1296)
        if dot1x.host_mode:
            if dot1x.host_mode.mode == "single-host":
                self._cli_line("dot1x host-mode single-host")
                ...
                self._cli_line(host_mode_cli)

        if dot1x.eapol.disabled is True:
            self._cli_line("dot1x eapol disabled")
        if dot1x.mac_based_access_list is True:
            self._cli_line("dot1x mac based access-list")
        ...
        if dot1x.mac_based_authentication.enabled is True:
            if dot1x.mac_based_authentication.host_mode_common is True:
                self._cli_line("dot1x mac based authentication host-mode common")
                if dot1x.mac_based_authentication.always is True:
                    self._cli_line("dot1x mac based authentication always")
            else:
                auth_cli = "dot1x mac based authentication"
                if dot1x.mac_based_authentication.always is True:
                    auth_cli += " always"
                self._cli_line(auth_cli)

        # timeout (J2 1317-1333)
        if dot1x.timeout:
            self._cli_line("dot1x timeout quiet-period {}", dot1x.timeout.quiet_period)
            if dot1x.timeout.reauth_timeout_ignore is True:
                self._cli_line("dot1x timeout reauth-timeout-ignore always")
            self._cli_line("dot1x timeout tx-period {}", dot1x.timeout.tx_period)
            self._cli_line("dot1x timeout reauth-period {}", dot1x.timeout.reauth_period)
            self._cli_line("dot1x timeout idle-host {} seconds", dot1x.timeout.idle_host)

        self._cli_line("dot1x reauthorization request limit {}", dot1x.reauthorization_request_limit)

        if dot1x.unauthorized.access_vlan_membership_egress is True:
            self._cli_line("dot1x unauthorized access vlan membership egress")
        if dot1x.unauthorized.native_vlan_membership_egress is True:
            self._cli_line("dot1x unauthorized native vlan membership egress")
    ...
    def _render_dot1x_aaa_unresponsive(self, dot1x: Any) -> None:
        ...
        for action_name, cli_part in (("phone_action", "phone action"), ("action", "action")):
            ...
            self._cli_line(aaa_action_config)

        if unresponsive.eap_response:
            self._cli_line(f"{aaa_config} eap response {unresponsive.eap_response}")
...
class EthernetInterfaceEncapsulationVlan(CliSection):
    ...
    def _section(self) -> None:
        ...
        self._section_heading("encapsulation vlan")
        self._cli_line(encapsulation_cli)
...
class EthernetInterfaceAddressLocking(CliSection):
    ...
    def _section(self) -> None:
        address_locking = self.intf.address_locking
        if address_locking.address_family.ipv4 is not None or address_locking.address_family.ipv6 is not None or address_locking.ipv4_enforcement_disabled is True:
            ...
            self._section_heading("address locking")
            if address_locking.address_family.ipv4 is True:
                self._cli_line("address-family ipv4")
            if address_locking.address_family.ipv6 is True:
                self._cli_line("address-family ipv6")
            if address_locking.address_family.ipv4 is False:
                self._cli_line("address-family ipv4 disabled")
            if address_locking.address_family.ipv6 is False:
                self._cli_line("address-family ipv6 disabled")
            if address_locking.ipv4_enforcement_disabled is True:
                self._cli_line("locked-address ipv4 enforcement disabled")
...
class EthernetInterfaceEvpnEthernetSegment(CliSection):
    ...
    def _section(self) -> None:
        evpn_ethernet_segment = self.intf.evpn_ethernet_segment
        if not evpn_ethernet_segment:
            return

        self._section_heading("evpn ethernet-segment")
        self._cli_line("identifier {}", evpn_ethernet_segment.identifier)
        self._cli_line("redundancy {}", evpn_ethernet_segment.redundancy)
        ...
        if designated_forwarder_election:
            if designated_forwarder_election.algorithm == "modulus":
                self._cli_line("designated-forwarder election algorithm modulus")
            ...
            if designated_forwarder_election.candidate_reachability_required is True:
                self._cli_line("designated-forwarder election candidate reachability required")
            elif designated_forwarder_election.candidate_reachability_required is False:
                self._cli_line("no designated-forwarder election candidate reachability required")

        self._cli_line("mpls tunnel flood filter time {}", evpn_ethernet_segment.mpls.tunnel_flood_filter_time)
        self._cli_line("mpls shared index {}", evpn_ethernet_segment.mpls.shared_index)
        self._cli_line("route-target import {}", evpn_ethernet_segment.route_target)
...
class EthernetInterfaceTxQueue(CliSection):
    ...
    def _section(self) -> None:
        ...
        self._section_heading(f"tx-queue {tx_queue.id}")
        if tx_queue.scheduler_profile_responsive is True:
            self._cli_line("scheduler profile responsive")
        if tx_queue.random_detect:
            ...
            if random_detect.ecn.threshold:
                ...
                self._cli_line(ecn_cmd)
            if random_detect.ecn.count is True:
                self._cli_line("random-detect ecn count")
...
class EthernetInterfaceUcTxQueue(CliSection):
    ...
    def _section(self) -> None:
        uc_tx_queue = self.uc_tx_queue
        self._section_heading(f"uc-tx-queue {uc_tx_queue.id}")
...
class EthernetInterfaceSyncE(CliSection):
    ...
    def _section(self) -> None:
        ...
        if intf.sync_e.enable is not True:
            return
        self._section_heading("sync-e")
        self._cli_line("priority {}", intf.sync_e.priority)
