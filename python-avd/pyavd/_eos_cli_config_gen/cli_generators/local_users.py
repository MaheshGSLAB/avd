...
class LocalUsersGenerator(CliGenerator):
    ...
    def _model(self) -> CliModel:
        return self.cli_config.local_users

    @cli_config_contributor
    def local_users(self) -> None:
        """Render all 'username ...' lines."""
        self._model.extend(LocalUsersBlock(self.data).render(indent=0))
...
class LocalUsersBlock(CliSection):
    ...
    def _generate(self) -> None:
        if not self.data.local_users:
            return
        hide = self.data.eos_cli_config_gen_configuration.hide_passwords
        for user in natural_sort(self.data.local_users, sort_key="name", ignore_case=False):
            name = user.name
            if user.disabled:
                self._header(f"no username {name}")
                continue
            ...
            if user.ssh_key:
                self._header(f"username {name} ssh-key {user.ssh_key}")
                if user.secondary_ssh_key:
                    self._header(f"username {name} ssh-key secondary {user.secondary_ssh_key}")
