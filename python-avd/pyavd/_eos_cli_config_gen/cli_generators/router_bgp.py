...
class RouterBgpBlock(CliSection):
    ...
    def _section(self) -> None:
        ...
        if bgp_as is None:
            return
        self._section_heading(f"router bgp {bgp_as}")
        ...
        for vlan in natural_sort(bgp.vlans or [], sort_key="id"):
            self._sub_section(RouterBgpVlan(vlan))

        for svc in natural_sort(bgp.vpws or [], sort_key="name"):
            self._sub_section(RouterBgpVpwsService(svc))

        for bundle in natural_sort(bgp.vlan_aware_bundles or [], sort_key="name"):
            self._sub_section(RouterBgpVlanAwareBundle(bundle))

        self._sub_section(RouterBgpRouteDistinguisher(bgp))
        self._sub_section(RouterBgpAddressFamilyEvpn(bgp))
        self._sub_section(RouterBgpAddressFamilyFlowSpec(bgp.address_family_flow_spec_ipv4, "ipv4"))
        self._sub_section(RouterBgpAddressFamilyFlowSpec(bgp.address_family_flow_spec_ipv6, "ipv6"))
        self._sub_section(RouterBgpAddressFamilyIpv4(bgp))
        self._sub_section(RouterBgpAddressFamilyIpv4LabeledUnicast(bgp))
        self._sub_section(RouterBgpAddressFamilyIpv4Multicast(bgp))
        self._sub_section(RouterBgpAddressFamilyIpv4SrTe(bgp))
        self._sub_section(RouterBgpAddressFamilyIpv6(bgp))
        self._sub_section(RouterBgpAddressFamilyIpv6Multicast(bgp))
        self._sub_section(RouterBgpAddressFamilyIpv6SrTe(bgp))
        self._sub_section(RouterBgpAddressFamilyLinkState(bgp))
        self._sub_section(RouterBgpAddressFamilyPathSelection(bgp))
        self._sub_section(RouterBgpAddressFamilyRtc(bgp))
        self._sub_section(RouterBgpAddressFamilyVpnIpv4(bgp))
        self._sub_section(RouterBgpAddressFamilyVpnIpv6(bgp))
        ...
        for tracker in natural_sort(bgp.session_trackers or [], sort_key="name"):
            self._sub_section(RouterBgpSessionTracker(tracker))
    ...
    def _render_global_settings(self, bgp: Any) -> None:
        ...
        """
        self._cli_line("bgp asn notation {}", bgp.as_notation)
        self._render_labeled_unicast_rib(bgp)
        self._cli_line("router-id {}", bgp.router_id)
        self._cli_line("update wait-for-convergence", bgp.updates.wait_for_convergence)
        self._cli_line("update wait-install", bgp.updates.wait_install)
        ...
        self._cli_line("bgp cluster-id {}", bgp.bgp_cluster_id)
        ...
        for bgp_default in bgp.bgp_defaults or []:
            self._cli_line(bgp_default)

        self._render_additional_paths(bgp)
        self._render_listen_ranges(bgp)

        self._cli_line("bgp bestpath d-path", bgp.bgp.bestpath.d_path)

        if bgp.neighbor_default.send_community == "all":
            self._cli_line("neighbor default send-community")
        elif bgp.neighbor_default.send_community is not None:
            self._cli_line(f"neighbor default send-community {bgp.neighbor_default.send_community}")
    ...
    def _render_peer_group(self, peer_group: EosCliConfigGen.RouterBgp.PeerGroupsItem) -> None:
        ...
        self._cli_line(f"neighbor {name} peer group")

        self._cli_line("neighbor {} remote-as {}", name, peer_group.remote_as)
        if peer_group.shutdown is True:
            self._cli_line(f"neighbor {name} shutdown")
        ...
        self._cli_line("neighbor {} local-as {} no-prepend replace-as", name, peer_group.local_as)
        self._cli_line("neighbor {} weight {}", name, peer_group.weight)
        if peer_group.passive is True:
            self._cli_line(f"neighbor {name} passive")
        self._cli_line("neighbor {} update-source {}", name, peer_group.update_source)

        self._render_bfd(name, peer_group.bfd, peer_group.bfd_timers)

        self._cli_line("neighbor {} description {}", name, peer_group.description)
        ...
        self._cli_line("neighbor {} ebgp-multihop {}", name, peer_group.ebgp_multihop)
        self._cli_line("neighbor {} ttl maximum-hops {}", name, peer_group.ttl_maximum_hops)
        if peer_group.route_reflector_client is True:
            self._cli_line(f"neighbor {name} route-reflector-client")
        self._cli_line("neighbor {} session tracker {}", name, peer_group.session_tracker)
        self._cli_line("neighbor {} timers {}", name, peer_group.timers)
    ...
    def _render_neighbor(self, neighbor: EosCliConfigGen.RouterBgp.NeighborsItem) -> None:
        ...
        self._cli_line("neighbor {} peer group {}", neighbor_ip_address, neighbor.peer_group)
        self._cli_line("neighbor {} remote-as {}", neighbor_ip_address, neighbor.remote_as)
        if neighbor.shutdown is True:
            self._cli_line(f"neighbor {neighbor_ip_address} shutdown")
        ...
        self._cli_line("neighbor {} local-as {} no-prepend replace-as", neighbor_ip_address, neighbor.local_as)
        self._cli_line("neighbor {} weight {}", neighbor_ip_address, neighbor.weight)
        if neighbor.passive is True:
            self._cli_line(f"neighbor {neighbor_ip_address} passive")
        self._cli_line("neighbor {} update-source {}", neighbor_ip_address, neighbor.update_source)
        ...
        self._cli_line("neighbor {} description {}", neighbor_ip_address, neighbor.description)
        ...
        self._cli_line("neighbor {} ebgp-multihop {}", neighbor_ip_address, neighbor.ebgp_multihop)
        self._cli_line("neighbor {} ttl maximum-hops {}", neighbor_ip_address, neighbor.ttl_maximum_hops)

        # Neighbors support negation for route-reflector-client; peer-groups do not.
        if neighbor.route_reflector_client is True:
            self._cli_line(f"neighbor {neighbor_ip_address} route-reflector-client")
        elif neighbor.route_reflector_client is False:
            self._cli_line(f"no neighbor {neighbor_ip_address} route-reflector-client")

        self._cli_line("neighbor {} session tracker {}", neighbor_ip_address, neighbor.session_tracker)
        self._cli_line("neighbor {} timers {}", neighbor_ip_address, neighbor.timers)
    ...
    def _render_redistribute_internal(self, bgp: EosCliConfigGen.RouterBgp) -> None:
        ...
        if bgp.bgp.redistribute_internal is True:
            self._cli_line("bgp redistribute-internal")
        elif bgp.bgp.redistribute_internal is False:
            self._cli_line("no bgp redistribute-internal")
    ...
    def _render_neighbor_interfaces(self, bgp: EosCliConfigGen.RouterBgp) -> None:
        ...
        for ni in natural_sort(bgp.neighbor_interfaces or [], sort_key="name"):
            if ni.peer_group is not None and ni.remote_as is not None:
                self._cli_line(f"neighbor interface {ni.name} peer-group {ni.peer_group} remote-as {ni.remote_as}")
            elif ni.peer_group is not None and ni.peer_filter is not None:
                self._cli_line(f"neighbor interface {ni.name} peer-group {ni.peer_group} peer-filter {ni.peer_filter}")
    ...
    def _render_bgp_default_flags(self, bgp: EosCliConfigGen.RouterBgp) -> None:
        ...
        if bgp.bgp.default.ipv4_unicast is True:
            self._cli_line("bgp default ipv4-unicast")
        elif bgp.bgp.default.ipv4_unicast is False:
            self._cli_line("no bgp default ipv4-unicast")

        if bgp.bgp.default.ipv4_unicast_transport_ipv6 is True:
            self._cli_line("bgp default ipv4-unicast transport ipv6")
        elif bgp.bgp.default.ipv4_unicast_transport_ipv6 is False:
            self._cli_line("no bgp default ipv4-unicast transport ipv6")
    ...
    def _render_graceful_restart(self, bgp: EosCliConfigGen.RouterBgp) -> None:
        ...
        if bgp.graceful_restart.enabled is not True:
            return
        self._cli_line("graceful-restart restart-time {}", bgp.graceful_restart.restart_time)
        self._cli_line("graceful-restart stalepath-time {}", bgp.graceful_restart.stalepath_time)
        self._cli_line("graceful-restart")
    ...
    def _render_graceful_restart_helper(self, bgp: EosCliConfigGen.RouterBgp) -> None:
        ...
        if bgp.graceful_restart_helper.enabled is False:
            self._cli_line("no graceful-restart-helper")
        elif bgp.graceful_restart_helper.enabled is True:
            if bgp.graceful_restart_helper.restart_time is not None:
                self._cli_line(f"graceful-restart-helper restart-time {bgp.graceful_restart_helper.restart_time}")
            elif bgp.graceful_restart_helper.long_lived is True:
                self._cli_line("graceful-restart-helper long-lived")
    ...
    def _render_additional_paths(self, bgp: EosCliConfigGen.RouterBgp) -> None:
        ...
        if bgp.bgp.additional_paths.receive is True:
            self._cli_line("bgp additional-paths receive")
        elif bgp.bgp.additional_paths.receive is False:
            self._cli_line("no bgp additional-paths receive")
        ...
        if send is None:
            return
        if send == "disabled":
            self._cli_line("no bgp additional-paths send")
        elif send == "ecmp" and send_limit is not None:
            self._cli_line(f"bgp additional-paths send ecmp limit {send_limit}")
        elif send == "limit" and send_limit is not None:
            self._cli_line(f"bgp additional-paths send limit {send_limit}")
        else:
            self._cli_line(f"bgp additional-paths send {send}")
    ...
    def _render_next_hop(
        ...
        if next_hop_self is True:
            self._cli_line(f"neighbor {entity_id} next-hop-self")
        if next_hop_peer is True:
            self._cli_line(f"neighbor {entity_id} next-hop-peer")
        if next_hop_unchanged is True:
            self._cli_line(f"neighbor {entity_id} next-hop-unchanged")
    ...
    def _render_as_path(self, entity_id: str, as_path: Any) -> None:
        ...
        if as_path.prepend_own_disabled is True:
            self._cli_line(f"neighbor {entity_id} as-path prepend-own disabled")
        if as_path.remote_as_replace_out is True:
            self._cli_line(f"neighbor {entity_id} as-path remote-as replace out")
    ...
    def _render_bfd(self, entity_id: str, bfd: bool | None, bfd_timers: Any, *, allow_negation: bool = False) -> None:
        ...
        """
        if bfd is True:
            self._cli_line(f"neighbor {entity_id} bfd")
            self._cli_line("neighbor {} bfd interval {} min-rx {} multiplier {}", entity_id, bfd_timers.interval, bfd_timers.min_rx, bfd_timers.multiplier)
        elif bfd is False and allow_negation:
            self._cli_line(f"no neighbor {entity_id} bfd")
    ...
    def _render_route_maps(self, entity_id: str, route_map_in: str | None, route_map_out: str | None) -> None:
        ...
        self._cli_line("neighbor {} route-map {} in", entity_id, route_map_in)
        self._cli_line("neighbor {} route-map {} out", entity_id, route_map_out)
    ...
    def _render_password_key(self, entity_id: str, password: str | None, password_type: str | None) -> None:
        ...
        hashed_password = hide_passwords(password, self.inputs.eos_cli_config_gen_configuration.hide_passwords)
        self._cli_line(f"neighbor {entity_id} password {pw_type} {hashed_password}")
    ...
    def _render_shared_secret(self, entity_id: str, shared_secret: Any) -> None:
        ...
        if shared_secret.profile is None or shared_secret.hash_algorithm is None:
            return
        self._cli_line(f"neighbor {entity_id} password shared-secret profile {shared_secret.profile} algorithm {shared_secret.hash_algorithm}")
    ...
    def _render_peer_tags(self, entity_id: str, peer_tag_in: str | None, peer_tag_out_discard: str | None) -> None:
        ...
        self._cli_line("neighbor {} peer-tag in {}", entity_id, peer_tag_in)
        self._cli_line("neighbor {} peer-tag out discard {}", entity_id, peer_tag_out_discard)
    ...
    def _render_remove_private_as(self, entity_id: str, remove_private_as: Any) -> None:
        ...
        if remove_private_as.enabled is True:
            ...
            self._cli_line(f"no neighbor {entity_id} remove-private-as")
    ...
    def _render_remove_private_as_ingress(self, entity_id: str, remove_private_as_ingress: Any) -> None:
        ...
        if remove_private_as_ingress.enabled is True:
            ...
            self._cli_line(f"no neighbor {entity_id} remove-private-as ingress")
    ...
    def _render_rib_in_pre_policy_retain(self, entity_id: str, rib_in: Any) -> None:
        ...
        if rib_in.enabled is True:
            ...
            self._cli_line(f"no neighbor {entity_id} rib-in pre-policy retain")
    ...
    def _render_send_community(self, entity_id: str, send_community: str | None) -> None:
        ...
        if send_community == "all":
            self._cli_line(f"neighbor {entity_id} send-community")
        elif send_community is not None:
            self._cli_line(f"neighbor {entity_id} send-community {send_community}")
...
class RouterBgpVrf(CliSection):
    ...
    vrf: EosCliConfigGen.RouterBgp.VrfsItem
    inputs: EosCliConfigGen

    def _section(self) -> None:
        vrf = self.vrf
        self._section_heading(f"vrf {vrf.name}")

        self._cli_line("rd {}", vrf.rd)
        self._cli_line("rd evpn domain {} {}", vrf.rd_evpn_domain.domain, vrf.rd_evpn_domain.rd)
        ...
        for address_family_entry in vrf.route_targets.field_import or []:
            for rt in address_family_entry.route_targets or []:
                self._cli_line(f"route-target import {address_family_entry.address_family} {rt}")
            if address_family_entry.address_family in ["evpn", "vpn-ipv4", "vpn-ipv6"]:
                if address_family_entry.rcf is not None:
                    if address_family_entry.vpn_route_filter_rcf is not None and address_family_entry.address_family in ["vpn-ipv4", "vpn-ipv6"]:
                        self._cli_line(f"route-target import {address_family_entry.address_family} rcf {address_family_entry.rcf} vpn-route filter-rcf {address_family_entry.vpn_route_filter_rcf}")
                    else:
                        self._cli_line(f"route-target import {address_family_entry.address_family} rcf {address_family_entry.rcf}")
                self._cli_line("route-target import {} route-map {}", address_family_entry.address_family, address_family_entry.route_map)

        for rt in natural_sort([r for r in vrf.route_targets.import_evpn_domains or [] if r.domain == "all"], sort_key="route_target"):
            self._cli_line(f"route-target import evpn domain {rt.domain} {rt.route_target}")
        ...
        for address_family_entry in vrf.route_targets.export or []:
            for rt in address_family_entry.route_targets or []:
                self._cli_line(f"route-target export {address_family_entry.address_family} {rt}")
            if address_family_entry.address_family in ["evpn", "vpn-ipv4", "vpn-ipv6"]:
                if address_family_entry.rcf is not None:
                    if address_family_entry.vrf_route_filter_rcf is not None and address_family_entry.address_family in ["vpn-ipv4", "vpn-ipv6"]:
                        self._cli_line(f"route-target export {address_family_entry.address_family} rcf {address_family_entry.rcf} vrf-route filter-rcf {address_family_entry.vrf_route_filter_rcf}")
                    else:
                        self._cli_line(f"route-target export {address_family_entry.address_family} rcf {address_family_entry.rcf}")
                self._cli_line("route-target export {} route-map {}", address_family_entry.address_family, address_family_entry.route_map)

        for rt in natural_sort([r for r in vrf.route_targets.export_evpn_domains or [] if r.domain == "all"], sort_key="route_target"):
            self._cli_line(f"route-target export evpn domain {rt.domain} {rt.route_target}")
        for rt in natural_sort([r for r in vrf.route_targets.export_evpn_domains or [] if r.domain == "remote"], sort_key="route_target"):
            self._cli_line(f"route-target export evpn domain {rt.domain} {rt.route_target}")

        self._cli_line("router-id {}", vrf.router_id)
        self._cli_line("update wait-for-convergence", vrf.updates.wait_for_convergence)
        self._cli_line("update wait-install", vrf.updates.wait_install)
        self._cli_line("timers bgp {}", vrf.timers)

        if vrf.graceful_restart.enabled is True:
            self._cli_line("graceful-restart restart-time {}", vrf.graceful_restart.restart_time)
            self._cli_line("graceful-restart stalepath-time {}", vrf.graceful_restart.stalepath_time)
            self._cli_line("graceful-restart")
        ...
        if vrf.bgp.additional_paths.install is True:
            self._cli_line("bgp additional-paths install")
        elif vrf.bgp.additional_paths.install_ecmp_primary is True:
            self._cli_line("bgp additional-paths install ecmp-primary")
        self._cli_line("bgp additional-paths receive", vrf.bgp.additional_paths.receive)
        ...
        self._sub_section(RouterBgpVrfAddressFamilyFlowSpec(vrf.address_family_flow_spec_ipv4, "ipv4"))
        self._sub_section(RouterBgpVrfAddressFamilyFlowSpec(vrf.address_family_flow_spec_ipv6, "ipv6"))
        self._sub_section(RouterBgpVrfAddressFamilyIpv4(vrf))
        self._sub_section(RouterBgpVrfAddressFamilyIpv4Multicast(vrf))
        self._sub_section(RouterBgpVrfAddressFamilyIpv6(vrf))
        self._sub_section(RouterBgpVrfAddressFamilyIpv6Multicast(vrf))
        self._sub_section(RouterBgpVrfEvpnMulticast(vrf))
    ...
    def _render_neighbor(self, neighbor: Any) -> None:
        ...
        if neighbor.next_hop_self is True:
            self._cli_line(f"neighbor {neighbor_ip_address} next-hop-self")
        if neighbor.next_hop_peer is True:
            self._cli_line(f"neighbor {neighbor_ip_address} next-hop-peer")
        if neighbor.shutdown is True:
            self._cli_line(f"neighbor {neighbor_ip_address} shutdown")
        ...
        if neighbor.remove_private_as.enabled is True:
            ...
            self._cli_line(f"no neighbor {neighbor_ip_address} remove-private-as")

        if neighbor.as_path.prepend_own_disabled is True:
            self._cli_line(f"neighbor {neighbor_ip_address} as-path prepend-own disabled")
        if neighbor.as_path.remote_as_replace_out is True:
            self._cli_line(f"neighbor {neighbor_ip_address} as-path remote-as replace out")
        self._cli_line("neighbor {} local-as {} no-prepend replace-as", neighbor_ip_address, neighbor.local_as)
        self._cli_line("neighbor {} weight {}", neighbor_ip_address, neighbor.weight)
        if neighbor.passive is True:
            self._cli_line(f"neighbor {neighbor_ip_address} passive")
        ...
        if neighbor.bfd is True:
            self._cli_line(f"neighbor {neighbor_ip_address} bfd")
            bfd_timers = neighbor.bfd_timers
            self._cli_line("neighbor {} bfd interval {} min-rx {} multiplier {}", neighbor_ip_address, bfd_timers.interval, bfd_timers.min_rx, bfd_timers.multiplier)
        elif neighbor.bfd is False and neighbor.peer_group is not None:
            self._cli_line(f"no neighbor {neighbor_ip_address} bfd")
        ...
        if neighbor.rib_in_pre_policy_retain.enabled is True:
            ...
            self._cli_line(f"no neighbor {neighbor_ip_address} rib-in pre-policy retain")
        ...
        self._cli_line("neighbor {} timers {}", neighbor_ip_address, neighbor.timers)
        self._cli_line("neighbor {} route-map {} in", neighbor_ip_address, neighbor.route_map_in)

        if neighbor.additional_paths.receive is True:
            self._cli_line(f"neighbor {neighbor_ip_address} additional-paths receive")
        self._render_neighbor_additional_paths_send(neighbor_ip_address, neighbor.additional_paths)

        self._cli_line("neighbor {} route-map {} out", neighbor_ip_address, neighbor.route_map_out)

        if neighbor.password is not None:
            hashed_password = hide_passwords(neighbor.password, self.inputs.eos_cli_config_gen_configuration.hide_passwords)
            pw_type = neighbor.password_type if neighbor.password_type is not None else "7"
            self._cli_line(f"neighbor {neighbor_ip_address} password {pw_type} {hashed_password}")
        ...
        if neighbor.send_community == "all":
            self._cli_line(f"neighbor {neighbor_ip_address} send-community")
        elif neighbor.send_community is not None:
            self._cli_line(f"neighbor {neighbor_ip_address} send-community {neighbor.send_community}")
        ...
        self._cli_line("neighbor {} peer-tag in {}", neighbor_ip_address, neighbor.peer_tag_in)
        self._cli_line("neighbor {} peer-tag out discard {}", neighbor_ip_address, neighbor.peer_tag_out_discard)
        ...
        if neighbor.remove_private_as_ingress.enabled is True:
            ...
            self._cli_line(f"no neighbor {neighbor_ip_address} remove-private-as ingress")
    ...
    def _render_neighbor_additional_paths_send(self, ip: str, additional_paths: Any) -> None:
        ...
        if send is None:
            return
        if send == "disabled":
            self._cli_line(f"no neighbor {ip} additional-paths send")
        elif send == "ecmp" and send_limit is not None:
            self._cli_line(f"neighbor {ip} additional-paths send ecmp limit {send_limit}")
        elif send == "limit":
            self._cli_line("neighbor {} additional-paths send limit {}", ip, send_limit)
        else:
            self._cli_line(f"neighbor {ip} additional-paths send {send}")
...
class RouterBgpVrfAddressFamilyFlowSpec(CliSection):
    ...
    address_family: EosCliConfigGen.RouterBgp.VrfsItem.AddressFamilyFlowSpecIpv4 | EosCliConfigGen.RouterBgp.VrfsItem.AddressFamilyFlowSpecIpv6
    protocol: str

    def _section(self) -> None:
        if not self.address_family:
            return
        self._section_heading(f"address-family flow-spec {self.protocol}")
        address_family = self.address_family
        self._cli_line("bgp missing-policy direction in action {}", address_family.bgp.missing_policy.direction_in_action)
        self._cli_line("bgp missing-policy direction out action {}", address_family.bgp.missing_policy.direction_out_action)
        for neighbor in natural_sort(address_family.neighbors or [], sort_key="ip_address"):
            if neighbor.activate is True:
                self._cli_line(f"neighbor {neighbor.ip_address} activate")
...
class RouterBgpVrfAddressFamilyIpv4(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family ipv4")
        ...
        self._cli_line("bgp missing-policy direction in action {}", address_family.bgp.missing_policy.direction_in_action)
        self._cli_line("bgp missing-policy direction out action {}", address_family.bgp.missing_policy.direction_out_action)
        self._cli_line("bgp additional-paths receive", address_family.bgp.additional_paths.receive)
    ...
    def _render_neighbor(self, neighbor: Any) -> None:
        neighbor_ip_address = neighbor.ip_address
        if neighbor.activate is True:
            self._cli_line(f"neighbor {neighbor_ip_address} activate")
        if neighbor.additional_paths.receive is True:
            self._cli_line(f"neighbor {neighbor_ip_address} additional-paths receive")
        self._cli_line("neighbor {} route-map {} in", neighbor_ip_address, neighbor.route_map_in)
        self._cli_line("neighbor {} route-map {} out", neighbor_ip_address, neighbor.route_map_out)
        self._cli_line("neighbor {} rcf in {}", neighbor_ip_address, neighbor.rcf_in)
        self._cli_line("neighbor {} rcf out {}", neighbor_ip_address, neighbor.rcf_out)
        self._cli_line("neighbor {} prefix-list {} in", neighbor_ip_address, neighbor.prefix_list_in)
        self._cli_line("neighbor {} prefix-list {} out", neighbor_ip_address, neighbor.prefix_list_out)
        self._render_neighbor_additional_paths_send(neighbor_ip_address, neighbor.additional_paths)
        ...
        if next_hop_ipv6.enabled is not None:
            if next_hop_ipv6.enabled:
                ...
                self._cli_line(f"no neighbor {neighbor_ip_address} next-hop address-family ipv6")
        self._cli_line("neighbor {} peer-tag in {}", neighbor_ip_address, neighbor.peer_tag_in)
        self._cli_line("neighbor {} peer-tag out discard {}", neighbor_ip_address, neighbor.peer_tag_out_discard)
    ...
    def _render_neighbor_additional_paths_send(self, ip: str, additional_paths: Any) -> None:
        ...
        if send == "disabled":
            ...
            self._cli_line(f"neighbor {ip} additional-paths send limit {send_limit}")
        else:
            self._cli_line(f"neighbor {ip} additional-paths send {send}")
...
class RouterBgpVrfAddressFamilyIpv4Multicast(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family ipv4 multicast")
...
class RouterBgpVrfAddressFamilyIpv6(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family ipv6")
...
class RouterBgpVrfAddressFamilyIpv6Multicast(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family ipv6 multicast")
...
class RouterBgpVrfEvpnMulticast(CliSection):
    ...
    def _section(self) -> None:
        if self.vrf.evpn_multicast is not True:
            return
        self._section_heading("evpn multicast")
        ...
        if dr_election_algorithm is not None:
            if dr_election_algorithm == "preference":
                ...
                self._cli_line("gateway dr election algorithm preference {}", preference_value)
            else:
                self._cli_line(f"gateway dr election algorithm {dr_election_algorithm}")
        self._sub_section(_RouterBgpVrfEvpnMulticastAddressFamilyIpv4(self.vrf))
...
class RouterBgpVlan(CliSection):
    ...
    vlan: EosCliConfigGen.RouterBgp.VlansItem

    def _section(self) -> None:
        vlan = self.vlan
        self._section_heading(f"vlan {vlan.id}")
        self._cli_line("rd {}", vlan.rd)
        self._cli_line("rd evpn domain {} {}", vlan.rd_evpn_domain.domain, vlan.rd_evpn_domain.rd)
        for rt in natural_sort(vlan.route_targets.both or []):
            self._cli_line(f"route-target both {rt}")
        for rt in natural_sort(vlan.route_targets.field_import or []):
            self._cli_line(f"route-target import {rt}")
        for rt in natural_sort(vlan.route_targets.export or []):
            self._cli_line(f"route-target export {rt}")
        ...
        for rt in natural_sort(vlan.route_targets.import_export_evpn_domains or [], sort_key="domain"):
            self._cli_line(f"route-target import export evpn domain {rt.domain} {rt.route_target}")
        for r in natural_sort(vlan.redistribute_routes or []):
            self._cli_line(f"redistribute {r}")
        for r in natural_sort(vlan.no_redistribute_routes or []):
            self._cli_line(f"no redistribute {r}")
...
class RouterBgpVpwsService(CliSection):
    ...
    vpws_service: EosCliConfigGen.RouterBgp.VpwsItem

    def _section(self) -> None:
        svc = self.vpws_service
        if svc.name is None:
            return
        self._section_heading(f"vpws {svc.name}")
        self._cli_line("rd {}", svc.rd)
        self._cli_line("route-target import export evpn {}", svc.route_targets.import_export)
        self._cli_line("mpls control-word", svc.mpls_control_word)
        self._cli_line("label flow", svc.label_flow)
        self._cli_line("mtu {}", svc.mtu)
        for hashed_password in natural_sort(svc.pseudowires or [], sort_key="name"):
            self._sub_section(RouterBgpVpwsPseudowire(hashed_password))
...
class RouterBgpVpwsPseudowire(CliSection):
    ...
    pseudowire: EosCliConfigGen.RouterBgp.VpwsItem.PseudowiresItem
    ...
    def _section(self) -> None:
        ...
        if hashed_password.name is None or hashed_password.id_local is None or hashed_password.id_remote is None:
            return
        self._section_heading(f"pseudowire {hashed_password.name}")
        self._cli_line(f"evpn vpws id local {hashed_password.id_local} remote {hashed_password.id_remote}")
...
class RouterBgpVlanAwareBundle(CliSection):
    ...
    def _section(self) -> None:
        bundle = self.bundle
        self._section_heading(f"vlan-aware-bundle {bundle.name}")
        self._cli_line("rd {}", bundle.rd)
        self._cli_line("rd evpn domain {} {}", bundle.rd_evpn_domain.domain, bundle.rd_evpn_domain.rd)
        ...
        for r in natural_sort(bundle.no_redistribute_routes or []):
            self._cli_line(f"no redistribute {r}")
        self._cli_line("vlan {}", bundle.vlan)
...
class RouterBgpRouteDistinguisher(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not assignment_auto:
            return
        self._section_heading("route-distinguisher")
        self._cli_line("assignment auto range {} {}", assignment_auto.range.start, assignment_auto.range.end)
        for address_family in natural_sort(assignment_auto.address_families or []):
            self._cli_line(f"assignment auto address-family {address_family}")
...
class RouterBgpAddressFamilyEvpn(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family evpn")
        self._cli_line("route export ethernet-segment ip mass-withdraw", address_family.route.export_ethernet_segment_ip_mass_withdraw)
        self._cli_line("route import ethernet-segment ip mass-withdraw", address_family.route.import_ethernet_segment_ip_mass_withdraw)
        self._cli_line("bgp additional-paths receive", address_family.bgp.additional_paths.receive)
        self._render_af_bgp_additional_paths_send(address_family.bgp.additional_paths)
        self._cli_line("bgp next-hop-unchanged", address_family.next_hop_unchanged)
        ...
        if rib_tokens:
            self._cli_line(f"next-hop mpls resolution ribs {' '.join(rib_tokens)}")
        ...
        self._cli_line("domain identifier {}", address_family.domain_identifier)
        self._cli_line("domain identifier {} remote", address_family.domain_identifier_remote)
        self._cli_line("next-hop resolution disabled", address_family.next_hop.resolution_disabled)
        if address_family.route.import_match_failure_action == "discard":
            self._cli_line("route import match-failure action discard")
        if address_family.neighbor_default.next_hop_self_received_evpn_routes.enable is True:
            ...
            self._cli_line(nhs_cli)

        if address_family.evpn_hostflap_detection.enabled is False:
            self._cli_line("no host-flap detection")
            ...
            if hfd_suffix:
                self._cli_line(f"host-flap detection{hfd_suffix}")
        ...
        if address_family.layer_2_fec_in_place_update.enabled is True:
            ...
            self._cli_line(l2_cli)

        self._cli_line("route import overlay-index gateway", address_family.route.import_overlay_index_gateway)

        for segment in natural_sort(address_family.evpn_ethernet_segment or [], sort_key="domain"):
            self._sub_section(RouterBgpAddressFamilyEvpnEthernetSegment(segment))
    ...
    def _render_af_evpn_peer_group(self, peer_group: Any) -> None:
        ...
        if peer_group.activate is True:
            self._cli_line(f"neighbor {name} activate")
        elif peer_group.activate is False:
            self._cli_line(f"no neighbor {name} activate")
        if peer_group.additional_paths.receive is True:
            self._cli_line(f"neighbor {name} additional-paths receive")
        self._cli_line("neighbor {} route-map {} in", name, peer_group.route_map_in)
        self._cli_line("neighbor {} route-map {} out", name, peer_group.route_map_out)
        self._cli_line("neighbor {} rcf in {}", name, peer_group.rcf_in)
        self._cli_line("neighbor {} rcf out {}", name, peer_group.rcf_out)
        if peer_group.default_route.enabled is True:
            ...
            self._cli_line(dr_cli)
        self._render_af_neighbor_additional_paths_send(name, peer_group.additional_paths)
        self._cli_line("neighbor {} peer-tag in {}", name, peer_group.peer_tag_in)
        self._cli_line("neighbor {} peer-tag out discard {}", name, peer_group.peer_tag_out_discard)
        ...
        if peer_group.domain_remote is True:
            self._cli_line(f"neighbor {name} domain remote")
    ...
    def _render_af_evpn_neighbor(self, neighbor: Any) -> None:
        ...
        if neighbor.activate is True:
            ...
            self._cli_line(f"no neighbor {neighbor_ip_address} activate")
        if neighbor.additional_paths.receive is True:
            self._cli_line(f"neighbor {neighbor_ip_address} additional-paths receive")
        self._cli_line("neighbor {} route-map {} in", neighbor_ip_address, neighbor.route_map_in)
        self._cli_line("neighbor {} route-map {} out", neighbor_ip_address, neighbor.route_map_out)
    ...
    def _render_af_neighbor_additional_paths_send(self, entity_id: str, additional_paths: Any) -> None:
        ...
        if send is None:
            return
        if send == "disabled":
            self._cli_line(f"no neighbor {entity_id} additional-paths send")
        elif send == "ecmp" and send_limit is not None:
            self._cli_line(f"neighbor {entity_id} additional-paths send ecmp limit {send_limit}")
        elif send == "limit":
            self._cli_line("neighbor {} additional-paths send limit {}", entity_id, send_limit)
        else:
            self._cli_line(f"neighbor {entity_id} additional-paths send {send}")
...
class RouterBgpAddressFamilyEvpnEthernetSegment(CliSection):
    ...
    segment: EosCliConfigGen.RouterBgp.AddressFamilyEvpn.EvpnEthernetSegmentItem

    def _section(self) -> None:
        segment = self.segment
        self._section_heading(f"evpn ethernet-segment domain {segment.domain}")
        self._cli_line("identifier {}", segment.identifier)
        self._cli_line("route-target import {}", segment.route_target_import)
...
class RouterBgpAddressFamilyFlowSpec(CliSection):
    ...
    address_family: EosCliConfigGen.RouterBgp.AddressFamilyFlowSpecIpv4 | EosCliConfigGen.RouterBgp.AddressFamilyFlowSpecIpv6
    protocol: str
    ...
    def _section(self) -> None:
        ...
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
            if peer_group.activate is True:
                self._cli_line(f"neighbor {peer_group.name} activate")
            elif peer_group.activate is False:
                self._cli_line(f"no neighbor {peer_group.name} activate")
        for neighbor in natural_sort(address_family.neighbors or [], sort_key="ip_address"):
            if neighbor.activate is True:
                self._cli_line(f"neighbor {neighbor.ip_address} activate")
...
class RouterBgpAddressFamilyIpv4(CliSection):
    ...
    def _section(self) -> None:
        ...
        for network in natural_sort(address_family.networks or [], sort_key="prefix"):
            if network.route_map is not None:
                self._cli_line(f"network {network.prefix} route-map {network.route_map}")
            elif network.rcf is not None:
                self._cli_line(f"network {network.prefix} rcf {network.rcf}")
            else:
                self._cli_line(f"network {network.prefix}")
    ...
    def _render_af_ipv4_peer_group(self, peer_group: Any) -> None:
        ...
        self._cli_line("neighbor {} rcf in {}", name, peer_group.rcf_in)
        self._cli_line("neighbor {} rcf out {}", name, peer_group.rcf_out)
        self._cli_line("neighbor {} prefix-list {} in", name, peer_group.prefix_list_in)
        self._cli_line("neighbor {} prefix-list {} out", name, peer_group.prefix_list_out)
...
class RouterBgpAddressFamilyIpv4LabeledUnicast(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family ipv4 labeled-unicast")
        self._cli_line("update wait-for-convergence", address_family.update_wait_for_convergence)
        ...
        self._cli_line("bgp additional-paths receive", address_family.bgp.additional_paths.receive)
        self._render_af_bgp_additional_paths_send(address_family.bgp.additional_paths)
        self._cli_line("bgp next-hop-unchanged", address_family.bgp.next_hop_unchanged)
        self._cli_line("neighbor default next-hop-self", address_family.neighbor_default.next_hop_self)
        ...
        if next_hop_ribs:
            ...
            for rib in next_hop_ribs:
                if rib.rib_type == "tunnel-rib-colored":
                    ...
                    rib_tokens.append(rib.rib_type)
            if rib_tokens:
                self._cli_line(f"next-hop resolution ribs {' '.join(rib_tokens)}")
        ...
        self._cli_line("lfib entry installation skipped", address_family.lfib_entry_installation_skipped)
        self._cli_line("label local-termination {}", address_family.label_local_termination)
        self._cli_line("graceful-restart", address_family.graceful_restart)
        ...
        if aigp_session:
            for session_type in ["ibgp", "confederation", "ebgp"]:
                if getattr(aigp_session, session_type, None) is True:
                    self._cli_line(f"aigp-session {session_type}")
    ...
    def _render_af_lu_entity(self, entity_id: str, entity: Any) -> None:
        if entity.activate is True:
            self._cli_line(f"neighbor {entity_id} activate")
        else:
            self._cli_line(f"no neighbor {entity_id} activate")

        if entity.additional_paths.receive is True:
            self._cli_line(f"neighbor {entity_id} additional-paths receive")

        if entity.graceful_restart is True:
            self._cli_line(f"neighbor {entity_id} graceful-restart")

        self._cli_line("neighbor {} graceful-restart-helper stale-route route-map {}", entity_id, entity.graceful_restart_helper.stale_route_map)
        self._cli_line("neighbor {} route-map {} in", entity_id, entity.route_map_in)
        self._cli_line("neighbor {} route-map {} out", entity_id, entity.route_map_out)
        self._cli_line("neighbor {} rcf in {}", entity_id, entity.rcf_in)
        self._cli_line("neighbor {} rcf out {}", entity_id, entity.rcf_out)
        ...
        if entity.next_hop_self_v4_mapped_v6_source_interface is not None:
            self._cli_line(f"neighbor {entity_id} next-hop-self v4-mapped-v6 source-interface {entity.next_hop_self_v4_mapped_v6_source_interface}")
        elif entity.next_hop_self_source_interface is not None:
            self._cli_line(f"neighbor {entity_id} next-hop-self source-interface {entity.next_hop_self_source_interface}")
        ...
        self._cli_line("neighbor {} peer-tag in {}", entity_id, entity.peer_tag_in)
        self._cli_line("neighbor {} peer-tag out discard {}", entity_id, entity.peer_tag_out_discard)

        if entity.aigp_session is True:
            self._cli_line(f"neighbor {entity_id} aigp-session")

        if entity.multi_path is True:
            self._cli_line(f"neighbor {entity_id} multi-path")
...
class RouterBgpAddressFamilyIpv4SrTe(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family ipv4 sr-te")
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
...
class RouterBgpAddressFamilyIpv6(CliSection):
    ...
    def _render_af_ipv6_entity(self, entity_id: str, entity: Any, bg_prefix_list: str | None) -> None:
        ...
        self._cli_line("neighbor {} rcf in {}", entity_id, entity.rcf_in)
        self._cli_line("neighbor {} rcf out {}", entity_id, entity.rcf_out)
        self._cli_line("neighbor {} prefix-list {} in", entity_id, entity.prefix_list_in)
        self._cli_line("neighbor {} prefix-list {} out", entity_id, entity.prefix_list_out)
    ...
    def _section(self) -> None:
        ...
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
            if peer_group.activate is True:
                ...
                self._cli_line(f"no neighbor {peer_group.name} activate")
            if peer_group.additional_paths.receive is True:
                self._cli_line(f"neighbor {peer_group.name} additional-paths receive")

        for neighbor in natural_sort(address_family.neighbors or [], sort_key="ip_address"):
            if neighbor.activate is True:
                self._cli_line(f"neighbor {neighbor.ip_address} activate")
            if neighbor.additional_paths.receive is True:
                self._cli_line(f"neighbor {neighbor.ip_address} additional-paths receive")
            self._cli_line("neighbor {} route-map {} in", neighbor.ip_address, neighbor.route_map_in)
            self._cli_line("neighbor {} route-map {} out", neighbor.ip_address, neighbor.route_map_out)
            self._cli_line("neighbor {} peer-tag in {}", neighbor.ip_address, neighbor.peer_tag_in)
            self._cli_line("neighbor {} peer-tag out discard {}", neighbor.ip_address, neighbor.peer_tag_out_discard)
...
class RouterBgpAddressFamilyIpv6SrTe(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family ipv6 sr-te")
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
            self._render_af_sr_te_entity(peer_group.name, peer_group)
        for neighbor in natural_sort(address_family.neighbors or [], sort_key="ip_address"):
            self._render_af_sr_te_entity(neighbor.ip_address, neighbor)
...
class RouterBgpAddressFamilyLinkState(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family link-state")
        self._cli_line("bgp missing-policy direction in action {}", address_family.bgp.missing_policy.direction_in_action)
        self._cli_line("bgp missing-policy direction out action {}", address_family.bgp.missing_policy.direction_out_action)
        ...
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
            if peer_group.activate is True:
                ...
                self._cli_line(f"no neighbor {peer_group.name} activate")
            self._cli_line("neighbor {} missing-policy direction in action {}", peer_group.name, peer_group.missing_policy.direction_in_action)
            self._cli_line("neighbor {} missing-policy direction out action {}", peer_group.name, peer_group.missing_policy.direction_out_action)

        for neighbor in natural_sort(address_family.neighbors or [], sort_key="ip_address"):
            if neighbor.activate is True:
                self._cli_line(f"neighbor {neighbor.ip_address} activate")
            self._cli_line("neighbor {} missing-policy direction in action {}", neighbor.ip_address, neighbor.missing_policy.direction_in_action)
            self._cli_line("neighbor {} missing-policy direction out action {}", neighbor.ip_address, neighbor.missing_policy.direction_out_action)

        path_selection = address_family.path_selection
        if path_selection:
            roles = path_selection.roles
            self._cli_line("path-selection", roles.producer)
...
class RouterBgpAddressFamilyPathSelection(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family path-selection")
        self._cli_line("bgp additional-paths receive", address_family.bgp.additional_paths.receive)
        self._render_af_bgp_additional_paths_send(address_family.bgp.additional_paths)
        ...
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
            ...
            if send is not None:
                if send == "disabled":
                    self._cli_line(f"no neighbor {peer_group.name} additional-paths send")
                elif send_limit is not None:
                    if send == "ecmp":
                        self._cli_line(f"neighbor {peer_group.name} additional-paths send ecmp limit {send_limit}")
                    elif send == "limit":
                        self._cli_line(f"neighbor {peer_group.name} additional-paths send limit {send_limit}")
                else:
                    self._cli_line(f"neighbor {peer_group.name} additional-paths send {send}")

        for neighbor in natural_sort(address_family.neighbors or [], sort_key="ip_address"):
            if neighbor.activate is True:
                self._cli_line(f"neighbor {neighbor.ip_address} activate")
            elif neighbor.activate is False:
                self._cli_line(f"no neighbor {neighbor.ip_address} activate")
            if neighbor.additional_paths.receive is True:
                self._cli_line(f"neighbor {neighbor.ip_address} additional-paths receive")
...
class RouterBgpAddressFamilyRtc(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family rt-membership")
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
            if peer_group.activate is True:
                self._cli_line(f"neighbor {peer_group.name} activate")
            elif peer_group.activate is False:
            ...
            if peer_group._get_defined_attr("default_route_target") is not Undefined:
                ...
                if default_rt is not None and default_rt.only is True:
                    self._cli_line(f"neighbor {peer_group.name} default-route-target only")
                else:
                    self._cli_line(f"neighbor {peer_group.name} default-route-target")
                if default_rt is not None and default_rt._get_defined_attr("encoding_origin_as_omit") is not Undefined:
                    self._cli_line(f"neighbor {peer_group.name} default-route-target encoding origin-as omit")
...
class RouterBgpAddressFamilyVpnIpv4(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family vpn-ipv4")
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
        ...
        if address_family.neighbor_default_encapsulation_mpls_next_hop_self.source_interface is not None:
            ...
            self._cli_line(f"neighbor default encapsulation mpls next-hop-self source-interface {src_iface}")
        self._cli_line("domain identifier {}", address_family.domain_identifier)
        if address_family.route.import_match_failure_action == "discard":
            self._cli_line("route import match-failure action discard")
...
class RouterBgpAddressFamilyVpnIpv6(CliSection):
    ...
    def _section(self) -> None:
        ...
        if not address_family:
            return
        self._section_heading("address-family vpn-ipv6")
        for peer_group in natural_sort(address_family.peer_groups or [], sort_key="name"):
            self._render_af_vpn_entity(peer_group.name, peer_group)
        for neighbor in natural_sort(address_family.neighbors or [], sort_key="ip_address"):
            self._render_af_vpn_entity(neighbor.ip_address, neighbor)
...
class RouterBgpSessionTracker(CliSection):
    ...
    tracker: EosCliConfigGen.RouterBgp.SessionTrackersItem

    def _section(self) -> None:
        tracker = self.tracker
        self._section_heading(f"session tracker {tracker.name}")
        self._cli_line("recovery delay {} seconds", tracker.recovery_delay)
